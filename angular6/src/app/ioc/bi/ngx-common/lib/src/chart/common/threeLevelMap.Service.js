...
var d3 = require('d3');
var $ = require("jquery");
var ThreeLevelMapService = /** @class */ (function () {
    function ThreeLevelMapService() {
        this.tipHeader = null;
        this.drillData = [];
        this.provinceData = {};
        this.mapData = {};
    }
    ThreeLevelMapService.prototype.getZoomScale = function (features, width, height) {
    ...
    ;
    ThreeLevelMapService.prototype.getCenters = function (features) {
        var longitudeMin = 100000;
        var latitudeMin = 100000;
        var longitudeMax = 0;
        var latitudeMax = 0;
    ...
    ;
    ThreeLevelMapService.prototype.fillColor = function (data, name) {
        var rowHeaderLength = 0, columnHeaderLength = 0, columnHeaderLink = [], columnMax, columnMin, columnList = [], dataValue = null;
    ...
    ;
    ThreeLevelMapService.prototype.tipData = function (data, name) {
        var rowHeaderLength = 0, columnHeaderLength = 0, tipsArray = [], columnHeaderLink = [];
        data ? data[0].map(function (d) {
            d.property == 'header_empty' ? rowHeaderLength++ : null;
        }) : null;
        data.map(function (d) {
        ...
        if (data[rowHeaderLength] && rowHeaderLength) {
            ...
            var index = 0;
            var tipslist = [];
            for (var t_1 = 0; t_1 < data.length; t_1++) {
                for (var j = 0; j < data[t_1].length; j++) {
                    if (data[t_1][j].data == name) {
                        index = j + 1;
                        tipslist = data[t_1];
                        break;
                    }
    ...
    ;
    ThreeLevelMapService.prototype.colorRange = function (args) {
        ...
        //add words
        var argText = args.svg.append("text");
        argText.attr({
            "class": "valueText",
            "x": args.width * 0.7 + 5,
        ...
        argText.text(function () { return args.contentHeader; });
    ...
    ;
    ThreeLevelMapService.prototype.backToTop = function (svg, width) {
        var that = this;
        svg.append('image')
            .attr('x', width * 0.7)
            .attr('y', 0)
    ...
    ;
    ThreeLevelMapService.prototype.map = function (options) {
        ...
        $.getJSON(mapPath, function (root) {
            ...
            svg.selectAll(".pathChina")
                ...
                var keyId = options.chartConfig.keys[0].id;
    ...
    ;
    ThreeLevelMapService.prototype.clickMap = function (argsProvince) {
    ...
    ;
    ThreeLevelMapService.prototype.drawPrivenceMap = function (argsProvince) {
        var background, that = this;
        $.getJSON(argsProvince.mapPath, function (root) {
            ...
            argsProvince.svg.selectAll(".pathProvince")
                ...
                .on("click", function (d) {
                var value = d.properties.name;
                var keyId = that.drillData[0].chartConfig.keys[1].id;
    ...
    ;
    ThreeLevelMapService.prototype.clickProvince = function (argsCountry) {
        d3.selectAll(".pathProvince").remove();
        d3.selectAll(".pathChina").remove();
        d3.selectAll(".d3-tip").remove();
    ...
    ;
    ThreeLevelMapService.prototype.drawCountyMap = function (argsCountry) {
        this.backColor = '';
        var that = this;
    ...
    ;
    ThreeLevelMapService.prototype.getRenderOption = function (option, drillConfig) {
        this.drillData = [];
        this.drillData.push(option);
        this.drillData.push(drillConfig);
    };
    ;
    ThreeLevelMapService.prototype.drawBubble = function (data, svg, root, projection) {
        var rowHeaderLength = 0, columnHeaderLength = 0, columnHeaderLink = [], columnMax, columnMin, columnList = [];
        data ? data[0].map(function (d) {
            d.property == 'header_empty' ? rowHeaderLength++ : null;
        }) : null;
        data.map(function (d) {
            ...
            for (var i = 0; i < data.length; i++) {
                if (data[0].length && (r.properties.name == data[i][rowHeaderLength - 1].data)) {
                    ...
                    svg.append("circle")
                        ...
                        d3.select(this)
                            .attr("stroke", "#449D44");
                        var tipsName = r.properties.name, tipsArray = [];
                        tip.html(function () {
                            for (var k = 0; k < data.length; k++) {
    ...
    ;
    ThreeLevelMapService = tslib_1.__decorate([
        core_1.Injectable(),
        tslib_1.__metadata("design:paramtypes", [])
    ], ThreeLevelMapService);
    return ThreeLevelMapService;
}());
exports.ThreeLevelMapService = ThreeLevelMapService;

//# sourceMappingURL=../../../sourcemaps/src/chart/common/threeLevelMap.Service.js.map
